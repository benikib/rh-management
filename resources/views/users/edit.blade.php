<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white">Modifier l'utilisateur</h2>
    </x-slot>
    ...
    <div class="bg-white rounded-xl shadow p-6 max-w-2xl   mx-auto">
        <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select id="role_id" name="role_id" class="search-select mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    ...
                    @foreach ($roles as $role)
                        <option value="{{ $role->id }}" @selected(old('role_id', $user->role_id) == $role->id)>{{ $role->nom }}</option>
                    @endforeach
                </select>
            ...
            @include('partials.form-actions', ['cancelRoute' => route('users.index'), 'submitLabel' => 'Mettre à jour'])
        </form>
    </div>
</x-app-layout>
